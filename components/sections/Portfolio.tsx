...
export default function Portfolio() {
  ...
  return (
    <section id="portafolio" ref={ref} className="section-padding relative">
      <NetworkPattern />
      <div className="container-custom relative z-10">
        ...
        <MobileCarousel itemsPerView={1.2} className="md:hidden mb-8">
          {portfolioItems.map((item, index) => (
            <div key={index} className={`${item.bgColor} p-5 sm:p-6 rounded-xl text-white relative shadow-lg min-h-[200px] flex flex-col`}>
              {item.highlight && (
                <div className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-white text-brand-darkGreen text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full shadow-lg z-10">
                  {item.highlight}
                </div>
              )}
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {portfolioItems.map((item, index) => (
            <motion.div
              ...
              className={`${item.bgColor} p-6 lg:p-8 rounded-2xl text-white relative transform hover:scale-[1.03] transition-all duration-300 flex flex-col min-h-[220px]`}
              style={{
